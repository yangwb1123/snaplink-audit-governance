...
// counter + backlog gauge follow pending (campaign
// fail-fast-or-warn-on-empty-rotated-ingest-token). The golden text is the
...
// new attempts_exhausted line, the pinned 11-line body, and the absence of
...
func TestMetricsEndpointServesPerCodeLines(t *testing.T) {
	handler := metricsHandler(func() kafka.ReplayerMetrics {
		return kafka.ReplayerMetrics{
			Replayed: 3, Permanent: 4, Unresolvable: 6, AttemptsExhausted: 5, RepublishFailures: 1,
		}
	})
