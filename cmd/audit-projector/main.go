...
func main() {
	...
	maxAttempts := flag.Int("max-attempts", intEnv("AUDIT_KAFKA_MAX_ATTEMPTS", 8), "max projection attempts per message before dead-lettering")
	dlqTopic := flag.String("dlq-topic", envOr("AUDIT_KAFKA_DLQ_TOPIC", kafka.TopicDLQ), "dead-letter topic; empty disables the DLQ publisher")
	flag.Parse()
	if *brokers == "" {
		log.Fatalf("brokers are required: pass -brokers or set AUDIT_KAFKA_BROKERS")
	}
	if *backoff <= 0 || *maxAttempts <= 0 {
		log.Fatalf("backoff and max-attempts must be positive")
	}
	logger := log.New(os.Stdout, "audit-projector ", log.LstdFlags|log.Lmicroseconds)
	// FIRST LINE: the resolved source topic and DLQ config are observable
	// even when ClickHouse/Kafka are unreachable (REQ-6, AC-3).
	logger.Printf("brokers=%s topic=%s group=%s clickhouse=%s max_attempts=%d dlq_topic=%s", *brokers, *topic, *group, *dsn, *maxAttempts, *dlqTopic)
	...
	consumer := kafka.NewConsumer(strings.Split(*brokers, ","), *topic, *group, kafka.IngestFunc(store.Insert), *backoff, logger, options...)
