...
B1-7 真实 IdP token 注入（G1 模式）：设置 `AUDIT_IDP_TOKEN_URL`/`AUDIT_IDP_CLIENT_ID`/
`AUDIT_IDP_CLIENT_SECRET`（可选 `AUDIT_IDP_SCOPE`）后，e2e 自动调用
`scripts/mint-token.sh` 铸造带 `tenant_id`/`scope` claims 的 JWT，并把栈切换到
**dev auth 关闭 + JWKS 验证**（dev token 负向断言 401；`AUDIT_JWKS_URL` 默认指向
宿主 IdP 的 host-gateway 别名，需 IdP 的 issuer 与 `AUDIT_JWT_ISSUER` 一致）。
未配置时过渡期使用 dev token 并告警。G1 模式已用真实 sso-server + 全栈
compose 实测通过（2026-08-07）。
