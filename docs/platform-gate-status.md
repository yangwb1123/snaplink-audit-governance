...
| **B2-6 激活门** | aero-id | ✅ | **2026-08-07 实施（cd65f57）**：`validateAuditGovernance` 拒绝 enabled+空 bindings（Load 时，env 不能绕过；禁用即 kill switch 接受空）；`TestAuditGovernanceActivationGate` 四 leg |
...
2. **aero-id（G3）**：B2 全项已实现（claim dispatch/contract suite/audit-replay/激活门），剩余为仓库门禁收口（注意：并行代理的 worktree 曾有接口漂移半成品，需其完成后跑 done-gate）。
