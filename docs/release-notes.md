...
**For developers:**
- `internal/archive/archive.go`: inside the existing `f.mu` critical section, `FileStore.Get` now `os.Lstat`s the resolved path and, when it exists but `!info.Mode().IsRegular()`, returns `nil, fmt.Errorf("archive path %s exists and is not a regular file", path)` — the same error class/string used by `Put`. A non-`IsNotExist` `Lstat` error is returned directly; `os.IsNotExist` falls through to `os.ReadFile`, preserving the missing-key contract. The `Lstat` sits under the held `f.mu`, so W1 torn-read serialization is unchanged.
- `containedPath`, the `Store` interface, the S3 store, and `internal/service/archive_fallback.go` (`archivedEventContext`) are intentionally untouched — this fix is contained entirely in the `FileStore.Get` read path. Out-of-scope items (S3 Get symlinks, tenant key-prefix scoping, an `ErrNotFound` sentinel, `containedPath`/`service` changes) remain separate directions.
- Regression/coverage: `internal/archive/archive_test.go` adds `TestFileStoreGetRejectsNonRegular` (AC-1/AC-2) mirroring `TestFileStorePutRejectsNonRegularPreExisting`, with `directory` (asserts `err != nil`, returned `[]byte` is `nil`) and `symlink` (creates a symlink inside the archive dir pointing to a sentinel file *outside* the root, `t.Skipf` on symlink-creation failure, asserts `err != nil`, `data == nil`, and that the symlink target bytes are not returned). `TestFileStoreGetMissing` (R3) and `TestFileStoreGetDoesNotObservePartialObject`/`TestFileStorePut*` (R4) remain satisfied.
- No config, schema, API, or DB migration change. Rollback = redeploy previous signed binary; durable archive bytes are unchanged. Single-commit revert is safe.
